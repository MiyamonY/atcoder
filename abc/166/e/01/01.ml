...
let () =
  let l = ArrayL.mapi arr ~f:(fun i a -> i + a) in
  let h = Hashtbl.create n in
  ArrayL.iteri arr ~f:(fun j a ->
      Hashtbl.find_option h (j-a)
      |> Option.map_default succ 1
      |> Hashtbl.add h (j - a));
  ArrayL.map l ~f:(fun a ->
      Hashtbl.find_option h a |> Option.map_default id 0)
  |> Array.sum
  |> Printf.printf "%d\n"
