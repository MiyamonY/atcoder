...
module List = struct
  ...
  let zip a b =
    ...
    | [], _ -> List.rev acc
    | _, [] -> List.rev acc
    | x::a, y::b -> aux a b @@ (x,y)::acc in
    aux a b []

  let mapi ~f ls =
    let rec aux i acc= function
      | [] -> List.rev acc
      | x::ls -> aux (succ i) ((f i x)::acc) ls in
    aux 0 [] ls
...
let scan_list ~sep cnv =
  Str.split (Str.regexp_string sep) @@ read_line ()
  |> List.map ~f:cnv
...
let n = scan "%d" id
let l = Array.of_list @@ scan_list ~sep:" " int_of_string

let () =
  Array.sort ~cmp:Int.compare l;
  let arr = Array.init (pred n) ~f:(fun _ -> 0) in
  List.iter ~f:(fun i -> arr.(i) <- l.(i+1) - l.(i)) (0++^Array.length arr);
  Array.mapi ~f:(fun i v -> (pred @@ Array.length l  - i)*(succ i)*v) arr;
  |> Array.sum 
  |> Printf.printf "%d\n"
