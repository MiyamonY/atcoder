...
let () =
  ...
  try List.iter ~f:(fun s ->
      let c = String.get s 0 in
      (if c = '!' then
         let ss = String.sub s 1 (pred (String.length s)) in
         (if Hashtbl.mem h ss then raise @@ Break ss)
       else
       if Hashtbl.mem h ("!" ^ s) then raise @@ Break s);
      Hashtbl.add h s true) ls;
    Printf.printf "satisfiable\n"
  with
  | Break s -> Printf.printf "%s\n" s
