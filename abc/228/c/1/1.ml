...
let () =
  ...
  let mat = Scan.matrix n 3 0 int_of_string in
  let arr = Array.make n 0 in
  let sorted = Array.make n 0 in
  Array.iteri arr ~f:(fun i _ -> arr.(i) <- mat.(i).(0) + mat.(i).(1) + mat.(i).(2);
                                 sorted.(i) <- arr.(i));
  Array.sort sorted ~cmp:(fun x y -> compare y x);
  Array.iter arr ~f:(fun v ->
      let w = v + 300 in
      let i = BinarySearch.array sorted ~f:(fun v -> w >= v) in
      Debug.p @@ Printf.sprintf "%d" i;
      if i < k then Printf.printf "Yes\n" else Printf.printf "No\n"
    )
