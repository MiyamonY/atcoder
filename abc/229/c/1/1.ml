...
let () =
  let (n, w) = Scan.scan "%d %d" tuple2 in
  let lines = Scan.lines n "%d %d" tuple2 |> List.sort ~cmp:(fun (a0, _) (a1, _) -> compare a1 a0) in
  List.fold_left lines ~init:(0, 0) ~f:(fun (sum, weight) (a, b) ->
      if w - weight > 0 then
        let diff = min (w - weight) b in
        (sum + a * diff, weight + diff)
      else
        (sum, weight))
  |> fun (sum, _) -> Printf.printf "%d\n" sum
