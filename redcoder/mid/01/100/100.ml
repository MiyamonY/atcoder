...
let () =
  try
    ListL.iter (1 ++ 1_000) ~f:(fun i ->
        if i * (pred i) / 2 = n then raise @@ Found i);
    Printf.printf "No\n"
  with
  | Found k ->
    let arr = Array.make_matrix k k 0  in
    let m = ref 1  in
    ListL.iter (0 ++^ k) ~f:(fun i ->
        ListL.iter (0 ++^ k) ~f:(fun j ->
            if i <> j && i < j then
              begin
                arr.(i).(j) <- !m;
                arr.(j).(i)  <- !m;
