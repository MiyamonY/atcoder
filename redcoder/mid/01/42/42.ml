...
let scan_lines n fmt f =
  if n = 0 then [| |]
  else Array.init n (fun _ -> scan fmt f)
...
let ds = scan_lines n "%d" id
let ms = scan_lines m "%d" id

let () =
  let dp = Array.make_matrix (m+1) (n+1)  (Int.max_num) in
  dp.(0).(0) <- 0;
  ListL.iter (1++m) ~f:(fun i ->
      ListL.iter (0++n) ~f:(fun j ->
          dp.(i).(j) <-
            if j = 0 then dp.(i-1).(j)
            else
              min dp.(i-1).(j)
                (if dp.(i-1).(j-1) < Int.max_num
                 then dp.(i-1).(j-1) + ms.(i-1)*ds.(j-1)
                 else Int.max_num)));
  ArrayL.map dp ~f:(fun d -> d.(n))
  |> Array.min
  |> Printf.printf "%d\n"
