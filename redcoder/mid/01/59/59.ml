...
let taxis = scan_lines n "%d %d" Tuple2.make
let edges =
  let e = Array.make n [] in
  ListL.iter (scan_lines k "%d %d" Tuple2.make) ~f:(fun (s,t)->
...
let rec wfs dists q=
  if not @@ Queue.is_empty q then
    ...
    ListL.iter edges.(v) ~f:(fun w ->
        if dists.(w) = - 1 then
          (dists.(w) <- dists.(v)+1;
           Queue.push w q));
    wfs dists q

let edges2 = Array.make n []

let rec solve dists h =
  if Heap.size h > 0 then
    let (d, v) = Heap.find_min h in
    solve dists
      (ListL.fold_left edges2.(v) ~init:(Heap.del_min h)
         ~f:(fun h (w, c) ->
             let dist = d+c in
             if dist < dists.(w) then
               (dists.(w) <- dist;
                Heap.add (dist, w) h)
             else h))
...
let () =
  ...
  ListL.iteri taxis ~f:(fun v (c, d) ->
      ListL.iter (0++^n) ~f:(fun i -> dists.(i) <- -1);
      dists.(v) <- 0; Queue.push v q;
      wfs dists q;
      ArrayL.iteri dists ~f:(fun i dist ->
          if 0 < dist && dist <= d then edges2.(v) <- (i,c)::edges2.(v)));
  ListL.iter (0++^n) ~f:(fun i -> dists.(i) <- Int.max_num);
  dists.(0) <- 0;
