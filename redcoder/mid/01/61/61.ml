...
let dp =
  let d = Array.make_matrix n n Int.max_num in
  ListL.iter (scan_lines m "%d %d %d" Tuple3.make) ~f:(fun (a, b, t) ->
      d.(pred a).(pred b) <- t;
      d.(pred b).(pred a) <- t;);
  ListL.iter (0++^n) ~f:(fun i-> d.(i).(i) <- 0);d
  ...
  ListL.iter (0++^n) ~f:(fun k->
      ListL.iter (0++^n) ~f:(fun i ->
          ListL.iter (0++^n) ~f:(fun j ->
              if dp.(i).(k) < Int.max_num && dp.(k).(j) < Int.max_num then
                dp.(i).(j) <- min dp.(i).(j) (dp.(i).(k) + dp.(k).(j))

            )
        )
    );
  ListL.map (0++^n) ~f:(fun i -> Array.max dp.(i))
  |> List.min
  |> Printf.printf "%d\n"
