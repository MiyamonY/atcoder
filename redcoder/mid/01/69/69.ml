...
let rec is_prime n i =
  ...
  else is_prime n (succ i)

let arr = Array.init (succ 100_000) (fun i ->
    if i <= 2 then 0
    else if i mod 2 = 1 && is_prime i 2 &&
            is_prime ((succ i)/2) 2 then 1 else 0)

let q = scan "%d" id

let () =
  ListL.iter (1 ++^ 100_000) ~f:(fun i ->
      arr.(i) <- arr.(i) + arr.(i-1));
  ListL.iter (scan_lines q "%d %d" Tuple2.make) ~f:(fun (l, r) ->
      Printf.printf "%d\n" @@ arr.(r) - arr.(l-1))
