...
let c = scan_list ~sep:' ' Int.of_string

let acc =
  let acc = Array.make n 0 in
  ListL.iter (1 ++^ n) ~f:(fun i ->
      let (+) n m = (n + m) mod prime in
      acc.(i) <- acc.(i-1) + pow a.(i-1) a.(i));
  acc

let () =
  ListL.map (List.combine (1::c) (c @ [1])) ~f:(fun (i, j) ->
      if i > j then acc.(pred i) - acc.(pred j)
      else acc.(pred j) - acc.(pred i))
  |> List.reduce (let (+) n m = (n+m) mod prime in (+))
  |> Printf.printf "%d\n"
