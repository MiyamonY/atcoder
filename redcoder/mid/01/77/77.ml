...
let a = Array.of_list @@ scan_lines (pred n) "%d" id
let ms = scan_lines m "%d" id

let () =
  ListL.iter (1 ++^ (Array.length a)) ~f:(fun i ->
      a.(i) <- a.(i) + a.(pred i));
  let arr = Array.append [|0|] a in
  ListL.fold_left ms ~init:(1, 0) ~f:(fun (cur, total) d ->
      let m = cur + d in
      let (+) n m = (n + m) mod 100_000 in
      let total = total + abs (arr.(pred @@ max m cur) - arr.(pred @@ min m cur)) in
      (m, total))
  |> Tuple2.second
  |> Printf.printf "%d\n"
