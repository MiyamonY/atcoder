...
let ls = scan_list (Some ' ') Int.of_string
let ms = Array.of_list @@ scan_lines (pred n) "%d %d %d" Tuple3.make

let () =
  let arr = Array.make n 0 in
  ListL.iter (List.combine (List.take (pred m) ls) (List.drop 1 ls)) ~f:(fun (a,b) ->
      arr.(pred @@ min a b) <- succ arr.(pred @@ min a b);
      arr.(pred @@ max a b) <- pred arr.(pred @@ max a b));
  ListL.iter (1 ++^ n) ~f:(fun i ->
      arr.(i) <- arr.(pred i) + arr.(i));
  ListL.map (0 ++^ (pred n)) ~f:(fun i ->
      let n = arr.(i) in
      let (a,b,c) = ms.(i) in
      min (n*a) (c + b*n))
  |> List.sum
