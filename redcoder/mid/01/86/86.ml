...
let routes = Array.of_list @@ scan_lines m "%d %d" Tuple2.make
...
let () =
  ListL.map (0 ++^ m) ~f:(fun k ->
      let uf = UnionFind.empty n in
      ArrayL.iteri routes ~f:(fun i (a,b)->
          if i <> k then
            UnionFind.unite uf (pred a) (pred b)
        );
      ListL.map (0++^n) ~f:(fun i ->
          ListL.map (0++^n) ~f:(fun j ->
              UnionFind.same uf i j
            )
        ) |> List.concat
      |> ListL.exists ~f:((=) false))
  |> ListL.map ~f:(fun s -> if s then 1 else 0)
  |> List.sum
  |> Printf.printf "%d\n"
