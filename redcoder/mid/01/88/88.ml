...
let () =
  ...
  ListL.iteri ls ~f:(fun i c ->
      (if i mod 2 = 0 then
         try
           let (c0, n) = Stack.top q in
           if c = c0 then
             (Stack.pop q |> ignore;
              Stack.push (c0, n+1) q)
           else Stack.push (c,1) q
         with
         | Stack.Empty -> Stack.push (c,1) q
       else
         let (c0, n) = Stack.pop q in
         if c = c0 then Stack.push (c0, n+1) q
         else
           try
             let (_,m) = Stack.pop q in
             Stack.push (c, 1+n+m) q
           with
           | Stack.Empty ->  Stack.push (c, n+1) q));
  Stack.enum q
  |> List.of_enum
  |> ListL.fold_left ~init:0 ~f:(fun pred (c,n) ->
      if c = 0 then pred + n else pred)
  |> Printf.printf "%d\n"
