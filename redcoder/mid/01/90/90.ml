...
let ns = scan_lines n "%f %f %f" Tuple3.make
let ms = scan_lines m "%f %f" Tuple2.make

let () =
  ListL.map ms ~f:(fun (x0, y0) ->
      ListL.map ns ~f:(fun (x1,y1,r) ->
          let dx = x0 -. x1 in
          let dy = y0 -. y1 in
          sqrt (dx*.dx+.dy*.dy) -. r
        ) @
      ListL.map ms ~f:(fun (x1,y1) ->
          if x0 = x1 && y0 = y1 then 200.0
          else
            let dx = x0 -. x1 in
            let dy = y0 -. y1 in
  ...
  |> List.concat
  |> List.append @@ ListL.map ns ~f:Tuple3.third
  |> List.min
  |> Printf.printf "%.10f\n"
