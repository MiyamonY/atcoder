...
let () =
  ListL.map (0++9) ~f:(fun i ->
      ListL.map (0++9) ~f:(fun j ->
          ListL.map (0++9) ~f:(fun k ->
              try
                let x =
                  String.find_from s 0 @@ String.of_char @@ Char.chr (i + Char.code '0') in
                let y = String.find_from s (succ x) @@ String.of_char @@ Char.chr (j + Char.code '0') in
                String.find_from s (succ y) @@ String.of_char @@ Char.chr (k + Char.code '0') |> ignore;
                1
              with
              | Not_found -> 0
            )
