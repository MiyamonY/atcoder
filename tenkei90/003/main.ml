...
module Array = struct
  ...
  let max arr = Array.fold_left ~f:(fun a v -> max a v) ~init:Int.min_int arr

  let min arr = Array.fold_left ~f:(fun a v -> min a v) ~init:Int.max_int arr

  let sum arr = Array.fold_left ~f:(+) ~init:0 arr
  ...
  let iscanl arr ~init ~f =
    Array.init (Array.length arr) ~f:(fun i ->
        if i = 0 then f init arr.(i)
        else f arr.(i-1) arr.(i))

  let to_assoc arr = Array.mapi arr ~f:(fun i v -> (i, v)) |> Array.to_list
end
...
let rec dfs g lengths i n =
  lengths.(i) <- n;
  List.iter g.(i) ~f:(fun j -> if lengths.(j) = min_num then dfs g lengths j (succ n))

let () =
  let n = Scan.scan "%d" Fun.id in
  let lines = Scan.lines (pred n) "%d %d" tuple2 in
  let g = Array.make n [] in
  List.iter lines ~f:(fun (a, b) ->
      g.(pred a) <- pred b :: g.(pred a);
      g.(pred b) <- pred a :: g.(pred b));
  let lengths = Array.make n min_num in
  dfs g lengths 0 0;
  let (u, _) = Array.to_assoc lengths |> List.max ~cmp:(fun (_, v) (_, w) -> compare w v) in
  let lengths = Array.make n min_num in
  dfs g lengths u 0;
  let (_, w) = Array.to_assoc lengths |> List.max ~cmp:(fun (_, v) (_, w) -> compare w v) in
  Printf.printf "%d\n" @@ succ w
