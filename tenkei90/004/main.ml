...
let () =
  ...
  let mat = Scan.matrix h w 0 int_of_string in
  let rows = Array.make h 0 in
  let cols = Array.make w 0 in
  List.iter (0++^h) ~f:(fun i ->
      rows.(i) <- Array.sum mat.(i);
    );
  List.iter (0++^w) ~f:(fun j ->
      cols.(j) <- List.sum @@ List.map (0++^h) ~f:(fun i -> mat.(i).(j))
    );
  List.iter (0++^h) ~f:(fun i ->
      List.iter (0++^w) ~f:(fun j ->
          if j <> 0 then Printf.printf " ";
          Printf.printf "%d" @@ rows.(i) + cols.(j) - mat.(i).(j);
        );
      Printf.printf "\n";
    )
