...
let () =
  let (n, k) = Scan.scan "%d %d" tuple2 in
  let s = Scan.scan "%s" Fun.id in
  let mat = Array.make_matrix ~dimx:(succ n) ~dimy:26 None in
  List.iter (n-1 -- 0) ~f:(fun i ->
      let c = Char.index (String.get s (i)) ~base:'a' in
      Array.blit ~src:mat.(i+1) ~src_pos: 0 ~dst:mat.(i) ~dst_pos:0 ~len:(Array.length mat.(i+1));
      mat.(i).(c) <-  Some i;
    );
  let rec aux j m acc =
    if m = k then String.join ~sep:"" @@ List.rev acc
    else
      let rec iter_char j c =
        let j' = mat.(succ j).(c) in
        if Option.is_some j' && n - Option.get j' >= k - m then (Char.from_index c ~base:'a', Option.get j')
        else
          iter_char j (succ c)
      in
      let (c, j') = iter_char j 0 in
      aux j' (succ m) (c::acc)
  in
  Printf.printf "%s\n" @@ aux (-1) 0 []
