...
let () =
  ...
  let a = Scan.array " " int_of_string in
  let q = Scan.scan "%d" Fun.id in
  let bs = Scan.lines q "%d" Fun.id in
  Array.sort a ~cmp:compare;
  List.iter bs ~f:(fun b ->
      let i = BinarySearch.array a ~f:(fun a -> a <= b) in
      Printf.printf "%d\n" @@ if i = 0 then Util.diff b  a.(0)
                              else if i = n then Util.diff b  a.(n-1)
                              else min (Util.diff a.(i)  b) @@ Util.diff b  a.(i-1))
