...
let () =
  let _ = Scan.scan "%d" Fun.id in
  let s = Scan.scan "%s" Fun.id in
  let dp = Array.make_matrix ~dimx:(succ @@ String.length "atcoder") ~dimy:(succ @@ String.length s) 0 in
  Array.iteri dp.(0) ~f:(fun i _ -> dp.(0).(i) <- 1);
  String.iteri "atcoder" ~f:(fun i c ->
      String.iteri s ~f:(fun j c' ->
          if c = c' then
            dp.(i+1).(j+1) <- (dp.(i+1).(j) + dp.(i).(j+1)) mod 1_000_000_007
          else
            dp.(i+1).(j+1) <- dp.(i+1).(j)
          )
    );
  Printf.printf "%d\n" @@ dp.(String.length "atcoder").(String.length s)
