...
let () =
  ...
  let arr = Scan.lines n "%d %d" tuple2 in
  let q = Scan.scan "%d" Fun.id in
  let arr2 = Scan.lines q "%d %d" tuple2 in
  let c1 = Array.make n 0 in
  let c2 = Array.make n 0 in
  List.iteri arr ~f:(fun i (c, p) ->
      if c = 1 then c1.(i) <- p
      else c2.(i) <- p);
  let acc1 = Array.scanl c1 ~init:0 ~f:(+) in
  let acc2 = Array.scanl c2 ~init:0 ~f:(+) in
  List.iter arr2 ~f:(fun (l, r) ->
