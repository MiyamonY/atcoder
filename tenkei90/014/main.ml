...
let () =
  ...
  let arr0 = Scan.array " " int_of_string in
  let arr1 = Scan.array " " int_of_string in
  Array.sort ~cmp:compare arr0;
  Array.sort ~cmp:compare arr1;
  Array.mapi arr0 ~f:(fun i v -> diff arr1.(i) v)
  |> Array.sum
  |> Printf.printf "%d\n"
