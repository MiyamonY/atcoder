...
let () =
  let t = Scan.scan "%f" Fun.id in
  let (l, x, y) = Scan.scan "%f %f %f" tuple3 in
  let q = Scan.scan "%d" Fun.id in
  List.iter (0++^q) ~f:(fun _ ->
      let e = Scan.scan "%f" Fun.id in
      Printf.printf "%.12f\n" @@
        if e = 0.0 then 0.0
        else
          let (y', z') = -. (l /. 2.0) *. Float.sin (2.0 *. Float.pi *. e /. t),
                         l /. 2.0 -. (l /. 2.0) *. Float.cos (2.0 *. Float.pi *. e /. t) in
          let r = Float.sqrt (Float.pow x 2.0 +. Float.pow (y -. y') 2.0) in
          360.0 *. Float.atan2 z' r /. (2.0 *. Float.pi)
    )
