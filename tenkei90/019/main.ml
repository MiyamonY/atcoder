...
let () =
  ...
  let dp = Array.make_matrix ~dimx:(succ @@ 2 * n) ~dimy:(succ @@ 2 * n) None in
  let rec aux l r =
    if l + 2 = r then diff arr.(pred r)  arr.(l)
    else
      if Option.is_some dp.(l).(r) then Option.get dp.(l).(r)
      else
        let x =
          let z = List.range (l + 2) (pred r) ~step:2 in
          List.map z ~f:(fun r' -> aux l r' + aux r' r)
          |> List.min ~cmp:compare
        in
        let y = aux (succ l) (pred r) + diff arr.(l) arr.(pred r) in
        dp.(l).(r) <- Some (min x y);
        Option.get dp.(l).(r)
  in
  Printf.printf "%d\n" @@ aux 0 (2*n)
