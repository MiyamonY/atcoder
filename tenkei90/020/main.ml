...
module Util = struct
  ...
  let rec pow x y = if y = 0 then 1 else x * pow x (pred y)
end
...
let () =
  let (a,b,c) = Scan.scan "%Ld %Ld %Ld" tuple3 in
  let rec test acc n =
    if n = Int64.zero then false
    else
      if a < Int64.mul c  acc then true
      else test (Int64.mul c  acc) (Int64.pred n)
  in
  if test Int64.one b then Printf.printf "Yes\n"
