...
let () =
  let (_, k) = Scan.scan "%d %d" tuple2 in
  let a = Scan.list " " int_of_string in
  let b = Scan.list " " int_of_string in
  let (sum, e) = List.fold_left (List.zip a b) ~init:(0, 0) ~f:(fun (sum, e) (a, b) ->
                     let d = diff a b in
                     (sum + d, e + (d mod 2)) ) in
  if sum <= k && e mod 2 = k  mod 2 then Printf.printf "Yes\n"
