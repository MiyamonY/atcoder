...
module Graph = struct
  ...
  let add_edge a b d g =
    g.(a) <- (d, b)::g.(a)

  let add_edge_mut a b d g =
    g.(a) <- (d, b)::g.(a);
    g.(b) <- (d, a)::g.(b)

  let reverse g =
    let g' = Array.make (Array.length g) [] in
    Array.iteri g ~f:(fun a l  ->
        List.iter l ~f:(fun (d, b) ->
            g'.(b) <- (d, a)::g'.(b)
          )
      );
    g'
...
let rec dfs visited n v g =
  visited.(n) <- Some v;
  List.iter g.(n) ~f:(fun (_, e) ->
      if Option.is_none visited.(e) then dfs visited e (not v) g)

let () =
  let n = Scan.scan "%d" Fun.id in
  let arr = Scan.lines (pred n) "%d %d" tuple2 in
  let g = Graph.make n in
  List.iter arr ~f:(fun (a,b) ->
      Graph.add_edge_mut (pred a) (pred b) 1 g);
  let visited = Array.make n None in
  dfs visited 0 true g;
  let visited' = Array.to_list visited in
  let ne = List.filter visited' ~f:(fun v -> Option.is_some v && Option.get v) |> List.length in
  let sv = ne >= n / 2 in
  List.mapi visited' ~f:tuple2
  |> List.filter  ~f:(fun (_, v) -> Option.is_some v && Option.get v = sv) |> List.take (n/2)
  |> List.map ~f:(fun (i, _) -> string_of_int @@ succ i)
  |> String.concat ~sep: " "
  |> Printf.printf "%s\n"
